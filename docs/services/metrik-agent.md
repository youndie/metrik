...
процессе. Переполнение — потеря замеров, но видимая: `trySend` отказывает, и отказ считает
`AgentCounters.dropped`. Отказ здесь обязателен, поэтому у канала **нет** `onBufferOverflow`: канал
