...
Образец для копирования во всём — [katcher](https://github.com/youndie/katcher): та же схема
...
Десятка стояла не по расчёту: это дефолт `sqlx`, который мы просто не трогали. Замер на стенде
(пять парных повторов, чередующиеся армы, одинаковая нагрузка) показал, что она обходится дорого и
не покупает ничего:

| | пул 10 | пул 2 |
|---|---|---|
| полка потребления | 192 МиБ | **133 МиБ** |
| пик под нагрузкой | 308 МиБ | **226 МиБ** |
| запросов в секунду | 2867 | **3168** |

Все пять пар легли в одну сторону по каждой строке. Причина в устройстве драйвера: `sqlx4k` —
обёртка над Rust-ным `sqlx`, и тот заводит **отдельный OS-поток на каждое соединение**, плюс на
каждое же — кэш страниц SQLite (около 2 МиБ) и кэш из сотни подготовленных выражений. Писатель в
SQLite при этом один, так что десять соединений делят один и тот же лок, добавляя только
переключения контекста.

Практическое следствие за пределами этой переменной: вся эта память лежит **вне кучи
Kotlin/Native**. Рычаги `kotlin.native.runtime.GC` ею не управляют — что и подтвердилось, когда ни
`targetHeapBytes`, ни `targetHeapUtilization`, ни `autotune` не дали воспроизводимого эффекта.
