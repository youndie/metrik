...
> Серые пунктирные колонки по краям — разрывы: минуты, за которые сервис ничего не прислал.
> Пунктира порога на графике нет, и это правильно: у `latency` здесь порог 2000 мс, а пик — 84 мс,
> то есть порог в шкалу не попадает. Раньше он всё равно рисовался у потолка и утверждал, что
> порог равен максимуму.
