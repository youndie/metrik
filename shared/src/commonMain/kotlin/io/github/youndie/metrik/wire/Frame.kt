...
/**
 * Что сам агент потерял за окно.
 *
 * Без этого поля потеря замеров на сервере **неотличима от падения нагрузки**: окно приходит
 * целым, в нём просто меньше запросов, и дашборд честно рисует провал RPS у сервиса, который на
 * самом деле обслужил столько же. Мониторинг врёт ровно в ту минуту, ради которой его ставили.
 *
 * Числа — **дельта за окно**, а не счётчик с начала процесса: так же устроен [GcSnapshot], и по
 * той же причине — вопрос всегда «сколько потеряно в этом окне», а не «сколько с прошлого
 * перезапуска». Цена на проводе — около 25 байт, и только в пакете `q = 0`.
 *
 * Едет **независимо от `system`**: системный срез выключается конфигом (`systemMetrics = false`),
 * а слепота к потерям от этой настройки зависеть не должна.
 */
@Serializable
public data class AgentSnapshot(
    /** Замеры, не влезшие в очередь агента. */
    @SerialName("d") val dropped: Int,
    /** Пакеты окна, которые не удалось отправить. */
    @SerialName("f") val sendFailures: Int,
    /** Пакеты, превысившие бюджет MTU. */
    @SerialName("o") val oversized: Int,
)
...
public data class Frame(
    ...
    /** Потери самого агента за это окно. `null` — агент старше этого поля, а не «потерь не было». */
    @SerialName("ag") val agent: AgentSnapshot? = null,
)
