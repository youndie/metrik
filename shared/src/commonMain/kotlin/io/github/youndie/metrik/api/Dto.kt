 ...
 *
 * [droppedSamples] — про то же самое, но с другого конца провода: замеры, которые агент не смог
 * положить в свою очередь и посчитал потерянными. Точка с ненулевым значением занижает **и** RPS,
 * и перцентили, причём молча: окно пришло целым, в нём просто меньше запросов, чем сервис
 * обслужил. Ноль — потерь не было; ноль же придёт от агента, который поле ещё не шлёт.
 */
@Serializable
