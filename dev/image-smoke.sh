...
# **Адрес резолвится в IPv4 руками, и это не перестраховка.** Приёмник слушает `0.0.0.0`, то есть
# IPv4; `/dev/udp/<имя>/<порт>` в bash отдаёт выбор адреса getaddrinfo, а тот по RFC 3484 ставит
# IPv6 первым — и на раннере GitHub `localhost` это `::1`. Отправка при этом **успешна**: UDP
# ничего не подтверждает, `printf` возвращает ноль, датаграмма уходит в никуда. Проверено обеими
# сторонами: с `localhost` → `::1` сервис в `/api/services` не появляется вовсе, с `127.0.0.1`
# появляется за секунду. Ровно так первый же прогон в CI покраснел на исправном образе.
#
# `getent` есть не везде (на macOS его нет) — тогда остаётся имя как дано: там контейнер обычно
# запущен с `-p`, и docker-proxy слушает обе семьи.
udp_host=$(getent ahostsv4 "$host" 2>/dev/null | awk 'NR == 1 { print $1 }')
...
printf '%s' "$frame" > "/dev/udp/$udp_host/$udp_port" ||
    fail "датаграмма не ушла на $udp_host:$udp_port"
...
fail "сервис не появился в /api/services после датаграммы на $udp_host:$udp_port — тракт приём → SQLite → чтение оборван: ${services:-пусто}"
